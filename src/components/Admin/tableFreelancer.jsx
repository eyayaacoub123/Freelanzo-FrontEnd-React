...
function TableFreelancer(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:5000/adminGetAllFreelancers');
...
const handleDelete = async (idfreelancer) => {
  try {
    const response = await axios.delete(`http://localhost:5000/deleteFreelancer/${idfreelancer}`);
    // Remove the deleted client from the data state
    setData(data.filter(item => item._id !== idfreelancer));
          ...
          <div className="col-12">
            <div className="card">
              <div className="card-header">
                <h3 className="card-title">All Freelancers</h3>
